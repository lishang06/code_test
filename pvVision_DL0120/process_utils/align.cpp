...
float FLT_EPSILON = 1.192093e-007;
...
//重复定义
bool caculate_rectified_points(
    ...
    float *final_points )
{
    ...
    for( int i = 0; i < points_num; ++i )
    {
        ...
        final_points[2 * i] =  (float) fx ;
        final_points[2 * i + 1] = (float) fy;
    }
    return true;
}

bool transformation_maker(int crop_width, int crop_height,const float *points, int points_num,
    const float *mean_shape, int mean_shape_width, int mean_shape_height,double *transformation, int N = 1 )
{
    // std::unique_ptr<float[]> raw_std_points( new float[points_num * 2] );
    // float *std_points = raw_std_points.get();
    float std_points[10]={0.0};
    for (int i = 0; i < points_num; ++i) {
    ...
    PADDING_TYPE ptype )
{
    ...
    if( final_points )
    {
        check3 = caculate_rectified_points( points, points_num,
                                        transformation,
...
bool face_crop_core(
    ...
    SAMPLING_TYPE type )
{
    return face_crop_core_executor(
