...
void gradMagNorm( float *M, float *S, int h, int w, float norm ) {
    __m128 *_MM, *_SS, _norm; int i=0, n=h*w, n4=n/4;
    _SS = (__m128*) S; _MM = (__m128*) M; _norm = sse::SET(norm);
    bool sse = !(size_t(M)&15) && !(size_t(S)&15);
    if(sse) for(; i<n4; i++) { *_MM=sse::MUL(*_MM,sse::RCP(sse::ADD(*_SS++,_norm))); _MM++; }
  ...
  int nOrients, int softBin, float clip )
{
  ...
  // compute unnormalized constrast sensitive histograms hello
  R1 = (float*) wrCalloc(wb*hb*nOrients*2,sizeof(float)*20);
  gradHist( M, O, R1, h, w, binSize, nOrients*2, softBin, true );
  // compute unnormalized contrast insensitive histograms
  R2 = (float*) wrCalloc(wb*hb*nOrients,sizeof(float)*20);
...
void fhog(float* I,float* H,int height,int width,int channel,int binSize, int nOrients, float clip, bool crop){
    float *M = new float[height*width], *O = new float[height*width];
    gradMag(I,M,O, height, width, channel, true);

    int h = height/binSize;
    int w = width/binSize;
    int d = nOrients*3+5;
...
void getTrackFhog(float* input, int h , int w, float*  out, int binSize){
    ...
    fhog(input,out,WIDTH,HEIGHT,DEPTH,binSize,nOrients,clip,crop);
}
...
void scaleFhog(float* I,float* H,int height,int width,int channel,int binSize, int nOrients, float clip, bool crop){
    float *M = new float[height*width], *O = new float[height*width];
    gradMag(I,M,O, height, width, channel, true);
    ...
    scaleFhog(M, O, H, height, width, binSize, nOrients, -1, clip );

//    float* Htmp = H+(h*w*nOrients)*3;
...
void getScaleTrackFhog(float* input, int h , int w, float*  out, int binSize){
    ...
    scaleFhog(input,out,WIDTH,HEIGHT,DEPTH,binSize,nOrients,clip,crop);
}
